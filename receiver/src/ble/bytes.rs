...
impl<'a, T: Copy + FromBytes<'a>> BytesOr<'a, T> {
    ...
    pub fn read(&self) -> T {
        match self.0 {
            Inner::Bytes(mut b) => {
                let t = match T::from_bytes(&mut b) {
                    Ok(t) => t,
                    Err(e) => panic!("BytesOr::read: decode of {:?} failed: {}", Hex(b), e),
                };
                assert!(b.is_empty());
                t
            }
        ...
        match &mut self.inner.0 {
            Inner::Bytes(b) => {
                if b.is_empty() {
                    ...
                    let data = *b;
                    Some(match T::from_bytes(b) {
                        Ok(t) => t,
                        Err(e) => panic!("next: decode failed: {}, bytes left: {:?}", e, Hex(data)),
                    })
                }
            }
